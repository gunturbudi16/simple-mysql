...
module.exports = {
  ...
  getAllBooks: (search, sortBy, sort, offset, limit) => {
    ...
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT b.*, g.name as genre FROM books b INNER JOIN genres g ON b.genre = g.id WHERE title LIKE CONCAT('%',?,'%') ORDER BY ${sortBy} ${sort} LIMIT ?, ?`,
        [search, offset, limit],
        (error, result) => {
          if (!error) {
            // console.log(result)
  ...
  getBookByID: id => {
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT b.*, g.name as genre FROM books b INNER JOIN genres g ON b.genre = g.id WHERE b.id = ?`,
        [id],
        (error, result) => {
          if (!error) {
            resolve(result);
